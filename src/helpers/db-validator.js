...
export const notExistentBedName = async (findBedroomName = '') => {
  const notExistBedroomName = await Bedroom.findOne({
    bedroomName: findBedroomName
  })
...
export const existentUserOrEmail = async (usernameOrEmail = '') => {
  if (usernameOrEmail.includes('@')) {
    ...
    if (!existEmail) {
      throw new Error(
        `The Email ${usernameOrEmail} wasn't found in the DataBase`
      )
    }
  } else {
    ...
    if (!existUsername) {
      throw new Error(
        `The Username ${usernameOrEmail} wasn't found in the DataBase`
      )
    }
  }
}
