...
export const listBedroom = async (req, res) => {
  ...
  const [total, bedroom] = await Promise.all([
    ...
    Bedroom.find(query).skip(Number(from)).limit(Number(limit))
  ])
...
export const deleteBedroom = async (req, res) => {
  ...
  await Bedroom.findOneAndUpdate({ bedroomName }, { status: 'CANCEL' })

  const bedroom = await Bedroom.findOne({ bedroomName })
